...
public class MonthlyCalendarView extends View {
	protected static final String TAG = MonthlyCalendarView.class
			.getSimpleName();
	...
	protected Paint weekdayText = new Paint(Paint.SUBPIXEL_TEXT_FLAG
			| Paint.ANTI_ALIAS_FLAG);
	protected Paint holidayText = new Paint(Paint.SUBPIXEL_TEXT_FLAG
			| Paint.ANTI_ALIAS_FLAG);
	...
	// ロケール舞に曜日名の配列を取得しておく
	protected static final String[] weekdays = symbols.getShortWeekdays();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		this.setMeasuredDimension(this.measureWidth(widthMeasureSpec),
	...
	private int measureWidth(int widthMeasureSpec) {
		...
		if (specMode == MeasureSpec.EXACTLY) {
			...
			// 規定のサイズにパディングを加算
			result = DEF_WIDTH + getPaddingLeft() + getPaddingRight();
	...
	private int measureHeight(int heightMeasureSpec) {
		...
		if (specMode == MeasureSpec.EXACTLY) {
			...
			// 規定のサイズにパディングを加算
			result = DEF_HEIGHT + getPaddingTop() + getPaddingBottom();
			if (specMode == MeasureSpec.AT_MOST) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < 8; i++) {
			// |
			canvas.drawLine(i * this.cellWidth, 0, i * this.cellWidth, height,
					this.light);
			// ||
			canvas.drawLine(i * this.cellWidth + 1, 0, i * this.cellWidth + 1,
					height, this.hilite);
			...
			// ==
			canvas.drawLine(0, offsetY + 1, width, offsetY + 1, this.hilite);
		...
		// Y軸方向でセンタリングする。
		// float alignY = this.cellHeight / 2 - (metricsForD.ascent +
		// metricsForD.descent) / 2;
		float alignY = (this.cellHeight / 4)
				- ((metricsForD.ascent + metricsForD.descent) / 4);
		for (int col = 0; col < 7; col++) {
			// 曜日で色を変える
			this.setForground(weekdayText, col);
			// 曜日描画
			canvas.drawText(weekdays[col + 1], (col * this.cellWidth) + alignX,
					alignY, weekdayText);
		}
		...
		// Y軸方向で上寄せする。
		alignY = this.cellHeight / 4
				- (metricsForD.ascent + metricsForD.descent) / 2;

		// 祭日名
		float holidayAlignX = this.cellWidth / 10;
		// Y軸方向で上寄せする。
		float holidayAlignY = this.cellHeight / 3
				- (metricsForH.ascent + metricsForH.descent) / 2;
				...
				if (this.matrix[row - 1][col] != null) {
					...
					if (this.matrix[row - 1][col].isHoliday) {
						canvas.drawText(this.matrix[row - 1][col].holidayName,
								(col * this.cellWidth) + holidayAlignX, offsetY
										+ holidayAlignY + alignY // ( row *
																	// this.cellHeight
																	// ) + hy +
																	// y
								, this.holidayText);
	...
	private boolean isToday(DateInfo dateInfo) {
		...
		if (year != dateInfo.year || month != dateInfo.month
				|| day != dateInfo.day)
			return false;
	...
	protected void calcCalendarMatrix() {
		...
		int startDay = ca.get(Calendar.DAY_OF_WEEK); // 曜日

		// 月末の日付を取得(次の月の初日-1日)
		ca.add(Calendar.MONTH, 1);
		ca.add(Calendar.DATE, -1);
		int lastDate = ca.get(Calendar.DATE);
		...
		for (int date = 1; date <= lastDate; date++) {
			this.matrix[row][column] = new DateInfo(ca.get(Calendar.YEAR),
					ca.get(Calendar.MONTH) + 1 // 月は0オリジン
					, date);
			if (column == 6) {
				row++;
				column = 0;
	 ...
	 *            calendarをセットします
	 */
	public void setCalendar(Calendar init, Calendar calendar) {
		this.calendar = calendar;
		this.calcCalendarMatrix();
		this.setToDay(init, calendar);
	}
	 ...
	 *            1～31までの日をセット
	 */
	public void setToDay(Calendar ini, Calendar cal) {
		for (int row = 0; row < 6; row++) {
			for (int col = 0; col < 7; col++) {
				if (this.matrix[row][col] != null) {
					if (this.matrix[row][col].day == cal
							.get(Calendar.DAY_OF_MONTH)) {
						// Calendar calendar = Calendar.getInstance();

						if ((ini.get(Calendar.YEAR) != cal.get(Calendar.YEAR))
								|| (ini.get(Calendar.MONTH) != cal
										.get(Calendar.MONTH))) {
							this.select(10, 10);
						} else {
							this.select(col, row + 1); // 0行目は見出し
						}
						Log.i("Selected row, col", col + ", " + row);
					}
				}
			}
		}
	...
	public DateInfo getSelectedDateInfo() {
		// Log.d(TAG, "getSelectedInfo: row=" + this.selRow + " col=" +
		// this.selCol );
		if (this.selRow == 0)
			return null;
